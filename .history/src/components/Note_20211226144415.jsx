...
import { ref, set, onValue, update, add } from 'firebase/database';
/* material ui */
...
/*       id: nanoid(), */
  ...
  addNoteToDB = () => {
    ...
    set(ref(db, `${note_id}-${time.replace(/:\d+ /, ' ')}`), { */
     update(ref(db, `${this.state.editId}`), {
     ...
     const newPostKey = pushchild((ref(db),`${this.state.editId}`)).key
     const updates = {};
  updates[`/${this.state.editId}/` + newPostKey] = postData;
  updates['/user-posts/' + uid + '/' + newPostKey] = postData;

  return update(ref(db), updates);
db().ref().child(`${this.state.editId}`).push(
  newDate: this.state.newDate,
)
    /*  set(ref(db, `${this.state.editId}`), {
      newDate: this.state.newDate,
     }); */
   };
